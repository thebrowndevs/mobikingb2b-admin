...
import {
    Clock, User, Info, FileText, CheckCircle2, XCircle, AlertCircle, PhoneCall, Edit, Plus, Truck
} from 'lucide-react'
import PCard from './custom/PCard'
...
export default function ActivityTimeline({ logs }) {
    ...
    return (
        <PCard className="bg-white p-6 shadow-sm border border-slate-100 rounded-xl">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3 mb-6">
                <Clock className="h-5 w-5 text-slate-700" />
                <div>
                    <h2 className="text-lg font-bold text-slate-800">Activity Log & Audit Trail</h2>
                    <p className="text-xs text-slate-400">Chronological history of all updates and operations performed on this record.</p>
                </div>
            </div>

            {activityLogs.length === 0 ? (
                <p className="text-sm text-slate-400 italic py-4 text-center">No activity history recorded.</p>
            ) : (
                <div className="relative pl-6 space-y-6 before:absolute before:left-3 before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100">
                    {activityLogs.map((log, idx) => {
                        ...
                        return (
                            <div key={log._id || idx} className="relative flex flex-col sm:flex-row sm:items-start gap-3">
                                {/* Timeline Dot */}
                                ...
                                {/* Main Log Details */}
                                <div className="flex-1 bg-slate-50/50 hover:bg-slate-50 border border-slate-100 p-4 rounded-xl transition-colors space-y-2">
