...
function GroupBrandsSheet({ open, onOpenChange, group, onSave, isSaving }) {
    const { brandsAdminPaginationQuery } = useBrands();
    ...
    // Fetch brands with search and page parameters
    const brandQuery = brandsAdminPaginationQuery({
        page,
        limit,
        searchQuery: debouncedSearch
    });

    const brandsList = brandQuery.data?.brands || [];
    const totalPages = brandQuery.data?.pagination?.totalPages || 1;
    const paginationRange = getPaginationRange(page, totalPages);

    // Load initial selection when sheet opens
    useEffect(() => {
        if (!open || !group) return;
        const ids = (group.brands ?? []).map(b => (typeof b === 'string' ? b : (b._id || b)));
        setSelectedIds(ids);

        const objs = ids.map(id => {
            const foundInGroup = (group.brands ?? []).find(b => typeof b === 'object' && b?._id === id);
            const foundInList = brandsList.find(b => (b._id || b) === id);
            return foundInGroup || foundInList || { _id: id, name: `Brand ID: ${id}` };
        });
    ...
    const handleSaveClick = async () => {
        ...
        try {
            ...
            console.error("Failed to save layout brand assignments:", err);
        }
    };

    const isLoading = brandQuery.isLoading || brandQuery.isFetching;
        ...
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent className="w-[95vw] md:min-w-[85vw] overflow-hidden pb-1 flex flex-col items-start justify-start gap-0">
                <SheetHeader className="pb-0">
                    <SheetTitle>{group?.heading || 'Group Brands'}</SheetTitle>
                    <SheetDescription>Select and drag to reorder brands displayed under this layout group.</SheetDescription>
                </SheetHeader>
                ...
                <div className="px-4 mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 w-full h-full pb-10 relative">
                    {isLoading && (
                        <div className="absolute inset-0 bg-white/75 backdrop-blur-xs z-50 flex flex-col items-center justify-center gap-3">
                            ...
                            <p className="text-slate-600 text-sm font-medium">Fetching brands, please wait...</p>
                        </div>
                    )}
                    ...
                    <div className="border border-bdr2 rounded-xl p-3 flex flex-col h-[78vh] bg-back2">
                        <div className="flex items-center gap-2 mb-3">
                            <Label className="min-w-0 flex-1 font-semibold text-xs text-slate-600">Available Brands</Label>
                            <input
                                type="text"
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                placeholder="Search by name..."
                                className="border border-bdr2 rounded-lg px-3 py-1 w-full md:w-2/3 text-xs bg-back1 focus:outline-none focus:ring-1 focus:ring-purple-500"
                            />
                        </div>

                        <div className="flex-1 overflow-auto pr-1">
                            {brandsList.length === 0 && (
                                <p className="text-xs text-slate-400 italic p-3 text-center">No brands found.</p>
                            )}
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                selected ? "border-purple-400 bg-purple-50/10" : "border-bdr2 hover:border-slate-300"
                                            )}
                                        >
                                            {item.image ? (
                                                <img
                                            ...
                                            <Button
                                                ...
                                                disabled={selected}
                                                className="h-8 w-8 text-purple-600 hover:bg-purple-100 hover:text-purple-700 rounded-lg disabled:opacity-50 shrink-0"
                                            >
                                                <Plus size={16} />
                                            </Button>
                                        </div>
                    ...
                    {/* RIGHT: Selected brands (Reorderable) */}
                    <div className="border border-bdr2 rounded-xl p-3 flex flex-col h-[78vh] bg-back2">
                        <div className="flex justify-between items-center mb-3">
                            <span className="text-xs font-bold text-slate-700">Assigned Brands ({visibleItems.length})</span>
                        ...
                        <div className="flex-1 overflow-auto pr-1">
                            {visibleItems.length === 0 ? (
                                <p className="text-xs text-slate-400 italic p-3 text-center">No brands assigned to this group yet.</p>
                            ) : (
...
export default GroupBrandsSheet;
