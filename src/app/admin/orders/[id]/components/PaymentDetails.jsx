            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {details.map(({ label, value }) => {
                    ...
                    return (
                        <div
                            key={label}
                            className={`${isCoupon ? 'bg-green-50 rounded-lg p-3' : ''}`}
                        >
                            <span
                                className={`block text-xs font-medium uppercase ${isCoupon ? 'text-green-700' : 'text-gray-400'
                                    }`}
                            >
                                {label}
                            </span>
                            ...
                            <span
                                className={`mt-1 block text-base font-semibold ${isCoupon ? 'text-green-700' : 'text-gray-900'
                                    }`}
                            >
                                {value ?? "-"}
                            </span>
